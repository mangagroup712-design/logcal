/* Shared data and helpers for Planly. */
const PLANLY_KEYS = {
    tasks: 'planly_tasks',
...
const DEFAULT_CATEGORIES = ['仕事', 'プライベート', 'その他'];
...
function toggleTask(id) {
    ...
    if (task) {
        task.done = !task.done;
        if (task.done) task.urgent = false;
    }
    saveTasks(tasks);
    return tasks;
}
...
async function showPlanlyNotification(title, body, data) {
    ...
    try {
        if ('serviceWorker' in navigator) {
            ...
            await registration.showNotification(title, {
                ...
                icon: 'icon.png',
                badge: 'icon.png',
                tag: data && data.tag ? data.tag : 'planly-task-reminder',
                renotify: true,
                data: data || {}
            });
            return true;
...
function startTaskReminderScheduler() {
    ...
    const tick = async function() {
        ...
        for (const plan of duePlans) {
            ...
            await showPlanlyNotification('Planly リマインダー', body, {
                type: 'task-reminder',
                taskId: plan.task.id,
                due: plan.task.due,
                time: plan.task.time,
                offset: plan.offset,
                tag: 'planly-task-' + plan.task.id + '-' + plan.offset
            });
        }
